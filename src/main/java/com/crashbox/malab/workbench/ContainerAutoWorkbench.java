...
import com.crashbox.malab.ai.SlotOutput;
...
{
    public ContainerAutoWorkbench(InventoryPlayer playerInventory, TileEntityAutoWorkbench workbench)
    {
...
//                .getX() + 0.5D, (double) this._pos.getY() + 0.5D, (double) this._pos.getZ() + 0.5D) <= 64.0D;
    ...
    private boolean addToMatrix(ItemStack stack)
    {
        ...
        {
            // Evenly distribute
            stack.stackSize = TileEntityAutoWorkbench.balanceStacks(matches, quantityFound, quantityAllowed, stack.stackSize);
            for (Slot slot : slots)
            {
                slot.onSlotChanged();
            }
